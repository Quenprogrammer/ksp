...
@Component({
  selector: 'app-homeproductrefactor',
  imports: [CommonModule,RouterLink],
  templateUrl: './homeproductrefactor.component.html',
  styleUrl: './homeproductrefactor.component.css'
})
export class HomeproductrefactorComponent {
  ...
  loadProducts() {
    const productsRef = collection(this.firestore, 'products');
    this.products$ = collectionData(productsRef, { idField: 'docId' });
  }
  async deleteProduct(docId: string) {
    try {
      await deleteDoc(doc(this.firestore, 'products', docId));
